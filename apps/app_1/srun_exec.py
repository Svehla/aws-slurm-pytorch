...
# current pwd is /shared/app_1/source_code
sys.path.append('.')
sys.path.append('./src/download_multinode_dataset')

from shared import create_prefixed_print, stream_command_output, debug_identify_instance
from src.download_multinode_dataset import download_dataset
print = create_prefixed_print('[compute_srun]')

print('xx')
stream_command_output('pwd')
debug_identify_instance()
...
dataset_path = '../train_datasets/mnist'
...
# fix broken sbatch file system -> sbatch-ing this script has problem with local imports
import sys
sys.path.append('/shared/app_1/source_code')
sys.path.append('/shared/app_1/source_code/src/download_multinode_dataset')

from shared import create_prefixed_print, stream_command_output, debug_identify_instance
debug_identify_instance()
import requests
import torchvision
print(requests, torchvision)
...
# this script si run when all resources are allocated but only one computeNode is running
# so its a little HW waste but we do not want to run training preparation on the head node
dataset_path = '/shared/app_1/train_datasets/mnist'
download_dataset(path=dataset_path, print=print)
